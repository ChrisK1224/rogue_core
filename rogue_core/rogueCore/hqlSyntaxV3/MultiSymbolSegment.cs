...
namespace rogueCore.hqlSyntaxV3
{
    public class MultiSymbolSegment<mycollection, myType> where mycollection : IMyList<myType>, new()
    {
        ...
        internal MultiSymbolSegment(SymbolOrder thsOrder, String input, string[] keys,Func<string, myType> NewItem, string cutoffSymbol = "", Func<string,string> GetPattern = null)
        {
            if(GetPattern == null)
            {
                GetPattern = GetOutsideQuotesPattern;
            }
            if (cutoffSymbol != "" && Regex.IsMatch(input, GetPattern(cutoffSymbol)))
            {
                int cutoffIndex = Regex.Match(input, GetPattern(cutoffSymbol)).Index;
        ...
        protected virtual void AddSegmentItem(mycollection ret, string key, String item)
        {
            ...
            //else
            //{
                ret.Add(key.ToUpper(), NewItem(item.Trim()));
            //}
        }
        ...
        static String GetOutsideQuotesPattern(String sep)
        {
            String keyStr = "";
            if (sep.Any(ch => !Char.IsLetterOrDigit(ch)))
            {
    ...
    {
        public MultiSymbolString(SymbolOrder thsOrder, string input, string[] keys) : base(thsOrder, input, keys, BaseRet) { }
        protected override void AddSegmentItem(mycollection ret, string key, string item)
        {
            ret.Add(key.ToUpper(), item.Trim());
        }
        static  string BaseRet(string txt) { return txt; }
    }
    public class MutliSegmentEnum
    {
        ...
        internal static String GetOutsideSingleQuotesPattern(String[] seps)
        {
            String keyStr = "";
            foreach (string sep in seps)
            {
    ...
    public class ListKeyValPairs<type> : List<KeyValuePair<string, type>>, IMyList<type>
    {
        public virtual type GetValue(String key)
        {
            return this.Where(x => x.Key == key).FirstOrDefault().Value;
            //return this[key];
        }
        public new void Add(string key, type value)
        {
    ...
    {
        myType NewSegmentItem(SelectHQLStatement metadata, string hqlSegment);
    }
