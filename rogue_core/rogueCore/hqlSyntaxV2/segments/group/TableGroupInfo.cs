...
namespace rogueCore.hqlSyntaxV2.segments.table
{
    public class TableGroupInfo
    {
        ...
        internal List<MultiRogueRow> Transformer(List<MultiRogueRow> rows, Dictionary<String, FilledTable> joinTables, Dictionary<string, LevelStatement> levelStatements)
        {
            switch (formatType)
            {
        ...
        //List<FilledSelectRow> HierarchyTableTransform(List<FilledSelectRow> allRows, Dictionary<String, FilledTable> joinTables, Dictionary<string, LevelStatement> grpTables)
        //{
        ...
        //            foreach (var thsColName in (new FilledHQLQuery("FROM COLUMNENUMERATIONS SELECT * WHERE COLUMN_OID = \"" + thsCol.BaseColumnID + "\"").IterateRows(act)))
        //            {
        ...
        void LoopPrintHierachy(MultiRogueRow topRow, int currLvl, bool printBase = false)
        {
    ...
    class ValueRogueRow : IRogueRow
    {
        ...
        public IRoguePair NewWritePair(IORecordID ownerTblID, string columnName, string value)
        {
            throw new NotImplementedException();
        }
